...
int CBuiltins::Execute(const CStdString& execString)
{
  ...
  else if (execute.Equals("DynDB"))
  {
    ...
    for (unsigned int i = 0; i < COUNT; i++)
    {
      start = XbmcThreads::SystemClockMillis();
      CFileItemList items;
      db.GetObjectsNav(items, map<string, long>(), 4);
      duration = XbmcThreads::SystemClockMillis() - start;
      CLog::Log(LOGDEBUG, "DYNDB: Loading from database took %d ms", duration);
      average += duration;
    }
    average /= COUNT;
    CLog::Log(LOGDEBUG, "DYNDB: Loading from database average: %d ms", average);
    ...
    for (unsigned int i = 0; i < COUNT; i++)
    {
      start = XbmcThreads::SystemClockMillis();
      CFileItemList items;
      if (!db.GetObjectsNav(items, map<string, long>(), 2))
      {
        CLog::Log(LOGDEBUG, "DYNDB: DLL failed to load tags from files");
        break;
      }
      duration = XbmcThreads::SystemClockMillis() - start;
      CLog::Log(LOGDEBUG, "DYNDB: DLL tag loading took %d ms", duration);
      average += duration;
    }
    average /= COUNT;
    CLog::Log(LOGDEBUG, "DYNDB: DLL tag loading average: %d ms", average);
