...
string CDynamicDatabase::PrepareIterator(bson_iterator *it, const string &type)
{
  bson_type itType = bson_iterator_next(it);
  if (IsInteger(type) || IsBool(type))
  {
    ...
    if (itType == BSON_BOOL)
      return PrepareSQL("%i", bson_iterator_bool(it)).c_str();
  }
...
/* static */
void CDynamicDatabase::DeserializeJSON(const string &strJson, int id, IDBInfoTag *obj)
{
  // Parse the JSON string into a CVariant
  string strContent = strJson;
  ...
  // Deserialize JSON data polymorphically
...
void CDynamicDatabase::InitializeMainTable(bool recreate /* = false */)
{
  ...
  if (recreate)
  {
    ...
    strSQL = PrepareSQL(
      "INSERT INTO %s (id%s, strContentBSON, strContentJSON) "
      "SELECT id%s, strContentBSON, strContentJSON "
      "FROM %s",
...
void CDynamicDatabase::DeclareOneToMany(const char *column, const char *type /* = "TEXT" */, bool index /* = false */)
{
  ...
  {
    CLog::Log(LOGERROR, "%s - Cannot place INDEX constraint on TEXT (column %s)", __FUNCTION__, column);
    index = false;
  }
...
void CDynamicDatabase::AddIndexInternal(const char *column, const char *type)
{
  ...
  // Populate the new column with value from the strContentBSON column
  strSQL = PrepareSQL("SELECT id%s, strContentBSON FROM %s", m_table, m_table);
        ...
        case BSON_ARRAY:
          {
            ...
            {
              if (subtype == BSON_DOUBLE || subtype == BSON_STRING || subtype == BSON_BOOL
                  || subtype == BSON_INT || subtype == BSON_LONG)
                items.push_back(subit);
              subtype = bson_iterator_next(subit);
            }
          }
          break;
...
int CDynamicDatabase::AddObject(const IDBInfoTag *obj, bool bUpdate /* = true */)
{
  ...
  try
  {
    ...
    idObject = (object["databaseid"].isNull() ? -1 : (int)object["databaseid"].asInteger());
...
int CDynamicDatabase::AddOneToManyItem(const string &parent, const string &type, const CVariant &value)
{
  ...
  if (m_pDS->num_rows() == 0)
  {
    ...
    strSQL = PrepareSQL(
      (
        "INSERT INTO %s (id%s, %s) "
        "VALUES (NULL, " + PrepareVariant(value, type) + ")"
      ).c_str(),
      parent.c_str(), parent.c_str(), parent.c_str()
    );
    m_pDS->exec(strSQL.c_str());
...
bool CDynamicDatabase::GetObjectByID(int idObject, IDBInfoTag *obj)
{
  ...
  try
  {
    strSQL = PrepareSQL("SELECT strContentBSON FROM %s WHERE id%s=%i", m_table, m_table, idObject);
...
bool CDynamicDatabase::GetObjectsNav(CFileItemList &items, map<string, long> predicates, int createFn)
{
  ...
  try
  {
    ...
    //  SELECT object.idobject, strContentBSON
    ...
    // Build the query
    if (createFn == 1)
      strSQL = PrepareSQL("SELECT %s.id%s, strContentJSON FROM %s ", m_table, m_table, m_table);
    else
      strSQL = PrepareSQL("SELECT %s.id%s, strContentBSON FROM %s ", m_table, m_table, m_table);
    ...
    if (m_pDS->query(strSQL.c_str()))
    {
      while (!m_pDS->eof())
      {
        ...
        CFileItemPtr pItem;
        if (createFn == 1)
          pItem = CFileItemPtr(CreateFileItemJSON(m_pDS->fv(1).get_asString(), m_pDS->fv(0).get_asInt()));
        else
          pItem = CFileItemPtr(CreateFileItemBSON(m_pDS->fv(1).get_asString(), m_pDS->fv(0).get_asInt()));
        items.Add(pItem);
        m_pDS->next();
      }
      m_pDS->close();
  ...
  try
  {
    if (killOrphansMwahahaha)
    {
      ...
      for (vector<Item>::const_iterator it = m_multiLinks.begin(); it != m_multiLinks.end(); it++)
      {
        ...
        // belong to an object not being deleted) in a subquery, and use these
        // to find the orphaned ones
        strSQL = PrepareSQL(
          ...
          "WHERE id%s NOT IN ("  // WHERE idItem NOT IN (
            "SELECT id%s "       //   SELECT idItem
            "FROM %s "           //   FROM objectLinkItem
            "WHERE id%s<>%i"     //   WHERE idItem<>idObject
          ")",                   // )
      ...
      // Next, the one-to-many items... FROM becomes a comma-separated list of index column names
      CStdString FROM;
      for (vector<Item>::const_iterator it = m_singleLinks.begin(); it != m_singleLinks.end(); it++)
