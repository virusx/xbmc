...
CDVDPlayerAuxiliaryVideo::CDVDPlayerAuxiliaryVideo(int id
                                , CDVDClock* pClock
                                , CDVDMessageQueue& parent)
: CThread(GetThreadName(id).c_str())
, m_messageQueue(GetMsgQueueName(id).c_str())
...
CDVDPlayerAuxiliaryVideo::~CDVDPlayerAuxiliaryVideo()
{
  StopThread();
  //g_VideoReferenceClock.StopThread();
}

bool CDVDPlayerAuxiliaryVideo::OpenStream(CDVDStreamInfo &hint)
{
...
void CDVDPlayerAuxiliaryVideo::OpenStream(CDVDStreamInfo &hint, CDVDVideoCodec* codec)
{
  ...
  {
    CLog::Log(LOGERROR, "CDVDPlayerAuxiliaryVideo::OpenStream - Invalid framerate %d, using forced 25fps and just trust timestamps", (int)m_fFrameRate);
    m_fFrameRate = 25;
  }
...
void CDVDPlayerAuxiliaryVideo::CloseStream(bool bWaitForBuffers)
{
...
double CDVDPlayerAuxiliaryVideo::GetOutputDelay()
{
...
void CDVDPlayerAuxiliaryVideo::OnStartup()
{
  m_iDroppedFrames = 0;
...
void CDVDPlayerAuxiliaryVideo::Process()
{
  ...
  while (!m_bStop)
  {
    ...
    else if (pMsg->IsType(CDVDMsg::GENERAL_FLUSH)) // private message sent by (CDVDPlayerAuxiliaryVideo::Flush())
    {
      if(m_pVideoCodec)
        m_pVideoCodec->Reset();
      picture.iFlags &= ~DVP_FLAG_ALLOCATED;
...
void CDVDPlayerAuxiliaryVideo::OnExit()
{
...
void CDVDPlayerAuxiliaryVideo::ProcessVideoUserData(DVDVideoUserData* pVideoUserData, double pts)
{
  // check userdata type
...
bool CDVDPlayerAuxiliaryVideo::InitializedOutputDevice()
{
#ifdef HAS_VIDEO_PLAYBACK
...
void CDVDPlayerAuxiliaryVideo::SetSpeed(int speed)
{
...
void CDVDPlayerAuxiliaryVideo::StepFrame()
{
  m_iNrOfPicturesNotToSkip++;
}

void CDVDPlayerAuxiliaryVideo::Flush()
{
...
int CDVDPlayerAuxiliaryVideo::GetLevel()
{
...
#ifdef HAS_VIDEO_PLAYBACK
void CDVDPlayerAuxiliaryVideo::ProcessOverlays(DVDVideoPicture* pSource, double pts)
{
...
int CDVDPlayerAuxiliaryVideo::OutputPicture(const DVDVideoPicture* src, double pts)
{
...
void CDVDPlayerAuxiliaryVideo::AutoCrop(DVDVideoPicture *pPicture)
{
...
void CDVDPlayerAuxiliaryVideo::AutoCrop(DVDVideoPicture *pPicture, RECT &crop)
{
...
std::string CDVDPlayerAuxiliaryVideo::GetPlayerInfo()
{
  std::ostringstream s;
...
int CDVDPlayerAuxiliaryVideo::GetVideoBitrate()
{
  return (int)m_videoStats.GetBitrate();
}

void CDVDPlayerAuxiliaryVideo::ResetFrameRateCalc()
{
...
void CDVDPlayerAuxiliaryVideo::CalcFrameRate()
{
