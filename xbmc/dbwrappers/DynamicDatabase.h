...
public:
  ...
  bool GetObjectsNav(CFileItemList &items, std::map<std::string, long> predicates = std::map<std::string, long>(),
    int createFn = 2); // 1 = JSON, 2 = BSON
  virtual CFileItem *CreateFileItemJSON(const std::string &strJson, int id) const = 0; // Callback to instantiate data
  virtual CFileItem *CreateFileItemBSON(const std::string &strBson, int id) const = 0;
...
protected:
  ...
  static        void DeserializeJSON(const std::string &strJson, int id, IDBInfoTag *obj);
  static        void DeserializeBSON(const std::string &strBson64, int id, IDBInfoTag *obj);

  /*!
   * Turn a variant or BSON iterator into a SQL value, quoted where appropriate (e.g. "1" and "'string'").
   */
  std::string PrepareVariant(const CVariant &value, const std::string &type);
  std::string PrepareIterator(bson_iterator *it, const std::string &type);
